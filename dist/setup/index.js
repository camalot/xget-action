...
function assetName(tag, platform) {
    const versionNum = tag.startsWith('v') ? tag.slice(1) : tag;
    return `${constants_1.TOOL_NAME}_${versionNum}_${platform.os}_${platform.arch}.${platform.ext}`;
}
function downloadUrl(tag, asset) {
    return `https://github.com/${constants_1.REPO_OWNER}/${constants_1.REPO_NAME}/releases/download/${tag}/${asset}`;
}
/** Downloads and extracts the xget release archive, returning the extracted directory. */
async function downloadAndExtract(tag, platform) {
    const asset = assetName(tag, platform);
    const url = downloadUrl(tag, asset);
    core.info(`Downloading ${asset} (${tag}) from ${url}`);
    const downloadPath = await tc.downloadTool(url);
    const extractedDir = platform.ext === 'zip'
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const cacheLib = __importStar(__nccwpck_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@actions/cache'"); e.code = 'MODULE_NOT_FOUND'; throw e; }())));
const core = __importStar(__nccwpck_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@actions/core'"); e.code = 'MODULE_NOT_FOUND'; throw e; }())));
...
const cache = __importStar(__nccwpck_require__(7377));
const constants_1 = __nccwpck_require__(7242);
...
const state = __importStar(__nccwpck_require__(7155));
const version_1 = __nccwpck_require__(311);
const xget_cli_1 = __nccwpck_require__(4000);
/** Stable directory a cache entry restores/saves to, independent of the temp dir used per-run. */
function toolInstallDir(tag, archKey) {
    const base = process.env.RUNNER_TOOL_CACHE || process.env.RUNNER_TEMP || '.';
    return path_1.default.join(base, 'xget-action', tag, archKey);
}
async function installXget(xgetVersion, token) {
    const platform = (0, platform_1.getPlatformInfo)();
    const { tag } = await (0, version_1.resolveXgetVersion)(xgetVersion, token);
    const archKey = `${platform.os}-${platform.arch}`;
    const existing = tc.find(constants_1.TOOL_NAME, tag, platform.arch);
    if (existing) {
        core.info(`Found cached ${constants_1.TOOL_NAME} ${tag} at ${existing}`);
        state.setCacheHit(true);
        return { toolPath: existing, version: tag };
    }
    const installDir = toolInstallDir(tag, archKey);
    const binaryPath = path_1.default.join(installDir, platform.binaryName);
    const cacheKey = `xget-${archKey}-${tag}`;
    if (cacheLib.isFeatureAvailable()) {
        const restoredKey = await cache.restore([installDir], cacheKey);
        if (restoredKey && fs_1.default.existsSync(binaryPath)) {
            core.info(`Restored ${constants_1.TOOL_NAME} ${tag} from cache (key: ${restoredKey})`);
            const cachedDir = await tc.cacheDir(installDir, constants_1.TOOL_NAME, tag, platform.arch);
            state.setCacheHit(true);
            return { toolPath: cachedDir, version: tag };
        }
    }
    const extractedDir = await (0, download_1.downloadAndExtract)(tag, platform);
    fs_1.default.mkdirSync(installDir, { recursive: true });
    fs_1.default.copyFileSync(path_1.default.join(extractedDir, platform.binaryName), binaryPath);
...
async function run() {
    try {
        ...
        const { toolPath, version } = await installXget(inputs.xgetVersion, inputs.token);
        core.addPath(toolPath);
        core.setOutput('xget-version', version);
        core.info(`Using xget ${version}`);
        const args = (0, xget_cli_1.buildXgetArgs)(inputs);
        const env = {};
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.isSemverPrerelease = isSemverPrerelease;
exports.resolveXgetVersion = resolveXgetVersion;
const core = __importStar(__nccwpck_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@actions/core'"); e.code = 'MODULE_NOT_FOUND'; throw e; }())));
const http_client_1 = __nccwpck_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@actions/http-client'"); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
...
function isSemverPrerelease(version) {
    ...
    const parsed = semver_1.default.parse(cleaned);
 ...
 */
async function resolveXgetVersion(requested, token) {
    if (!requested || requested === 'latest') {
        const http = new http_client_1.HttpClient('xget-action');
        const url = `https://api.github.com/repos/${constants_1.REPO_OWNER}/${constants_1.REPO_NAME}/releases/latest`;
        const res = await http.getJson(url, authHeaders(token));
        if (!res.result?.tag_name) {
            throw new Error(`could not determine the latest ${constants_1.REPO_NAME} release`);
        }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.buildXgetArgs = buildXgetArgs;
